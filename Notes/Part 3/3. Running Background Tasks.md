...
- **Additional Feature**
  ...
  - Celery can schedule **periodic tasks** (like [cron jobs](https://www.google.com/search?q=cron+jobs)).
...
1. **Create `celery.py` besides `settings.py`**
...
2. **Configure Celery settings in `settings.py`**
   ...
   (`1` is the name of the DB, it's convention.)
...
> While development, if we create a task but the celery worker was already running, and we go to the url, then celery raises an error (`Received unregistered task`). So, we need to restart the celery worker after any new task addition.
