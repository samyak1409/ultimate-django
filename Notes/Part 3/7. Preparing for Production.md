...
- **Handlers** (decide what to do with logs):
  ...
  - `file` → `logging.FileHandler` (writes to e.g. `general.log` using `filename`).
...
4. Move env-specific configs out of `common.py` to both `dev.py`, `prod.py`:
   ...
   - **Middlewares** → keep in common; dev: insert `DebugToolbar` at 0, append `Silky` (See: [Order of Middlewares](https://github.com/samyak1409/ultimate-django/blob/main/Notes/Part%203/7.%20Preparing%20for%20Production.md#order-of-middlewares))
...
- Modify files that reference `DJANGO_SETTINGS_MODULE`:
  ...
  In `manage.py`, `pytest.ini`, `asgi.py`, `celery.py`, `wsgi.py`:
...
- To run: `python -m gunicorn storefront.wsgi` (instead of `gunicorn storefront.wsgi`, [why](https://g.co/gemini/share/f5dad9203d53))
