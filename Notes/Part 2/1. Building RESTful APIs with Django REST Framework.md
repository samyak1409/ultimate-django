...
Similar as above implementation, only differences are

1. Using following in views:

   ```py
       Collection.objects.annotate(product_count=Count("product"))
   ```

2. And following in serializers:
...
These add a read-only `product_count` field to the API.
