...
We can also change with what name existing fields of DB show in API:
...
One use-case is: internal name is changed, but if we change the external field usage name, all existing code of our current users would break, we can avoid that using this.
...
- Hyperlink (link to related's field's API endpoint)
...
Deserialize: Get model object from `dict` which we get from `request.data`.
...
> And this brings up a form to `POST` on the endpoint, that's the beauty of DRF's browseable API.
...
- `serializer.is_valid(raise_exception=True)`: Run the validations (when defining <ModelName>Serializer class, DRF automatically generate the rules on the basis of `models.<Field>`).

- `serializer.validated_data`: Again `dict`, but first, we know this data is valid as per our DB. Second, this `dict` contains related field(s) as an object.

### Customizing Validations

Consider the case where we want `password` field to be matched with `confirm_password` field, if not matched then the data is not valid, we'd need to override `validate` method in `<ModelName>Serializer` class:
...
Suppose if want to delete a product:
...
Determine: [wikipedia.org/wiki/List_of_HTTP_status_codes](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes) or [httpstatuses.io](https://httpstatuses.io)
