...
- Add `items` field in `OrderSerializer` using `OrderItemSerializer(many=True)`.
...
1. **Override** `get_permissions()` in the viewset instead using `permission_classes`.
...
> Mosh mistakenly left `'post'` out from `http_method_names`.
...
- Add `read_only=True` to `OrderSerializer.OrderItemSerializer`.

**Should've definitely worked** given we've used exactly this in the past as well, but not working for some weird reason. Tried to find why but got nothing. ([Gemini](https://g.co/gemini/share/67a99410815d))
...
- Override `save()` in `CustomUserCreateSerializer` to create a `Customer` when a `User` is saved.
...
- Every Django model fires built-in signals (e.g., `post_save`), but you can also create **custom signals** for app-specific events.
- Example: Fire an `order_created` signal whenever a new order is placed in the **store** app.
...
2. **Fire the signal in `CreateOrderSerializer`**:

   - In the end (after order creation & cart deletion):
...
- Any app (e.g., **core**) listening to it receives the `order` object and can react.
