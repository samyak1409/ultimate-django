...
- You want easy revocation of logins without complex token blacklisting.
...
REST APIs are **stateless** because each request from the client to the server must contain **all the information needed** for the server to understand and process it — the server does **not** remember anything about previous requests from that client.
...
- Saves time by avoiding manual implementation in every project.
...
- Restrict **modifications** to certain resources to **admin users**.
- Allow **read-only** access to anyone (including anonymous users) where appropriate.

### Problem

- Products endpoint is open to everyone for all actions.
- Need:
  - Products & collections: Anyone can read, only admins can modify.
  - Shopping carts: Anyone can create.
  - Customers: Only admins manage customers; authenticated users can access their own profile.

### Solution: Custom Permissions

Create `permissions.py` in the app.

> Take a look at definitions of built-in permissions like `rest_framework.permissions.IsAuthenticated`.
...
- No restrictions (anyone can create a cart).

#### Customers

- Switch to `ModelViewSet` for all operations.
- Default permission:
...
- Anonymous user:
  ...
  - Products & collections: Can view, cannot modify.
  - Customers: No access.

- Admin user:

  - Full control over products, collections, and customers.

- Authenticated non-admin:

  - Can access their own customer record.
