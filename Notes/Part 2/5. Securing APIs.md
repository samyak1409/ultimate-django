...
> Mosh didn't explain how does a JWT is verified on the server as it doesn't store anything?  
> → Django secret key is used to sign the JWT.
> 
> "The `SECRET_KEY` is used for **cryptographic signing** to protect your application's data from tampering. It's the foundation for securing sessions, forms (CSRF protection), and other signed data." - [Gemini](https://g.co/gemini/share/a2cb952628e5)
> 
> Now the working became clear.
...
The signature protects against **Tampering**, not **Theft**.

- If a hacker steals a JWT, they **can** impersonate the user.

- **The Signature's Job:** It prevents **Tampering** (e.g., changing your User ID or Role inside the token), not **Theft**.

- **The Defense:** To prevent theft, we use **HTTPS** (to stop sniffing) and **HttpOnly Cookies** (to stop XSS scripts from stealing it), combined with **short expiration times**.

\- [Gemini](https://gemini.google.com/share/6617ce109ce0)
...
But as you might remember, in [4. Django Authentication System > Groups and Permissions](/Notes/Part%202/4.%20Django%20Authentication%20System.md#groups-and-permissions), we created a `Customer Service` group in Django admin.
...
These `add`, `view`, `change`, `delete` permissions are [Built-in Permissions](/Notes/Part%202/4.%20Django%20Authentication%20System.md#built-in-permissions).
...
So, first we need to do this: [Creating Custom Permissions](/Notes/Part%202/4.%20Django%20Authentication%20System.md#creating-custom-permissions)
