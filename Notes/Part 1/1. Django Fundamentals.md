...
> Explains frontend vs backend, HTTP requests/responses, server-rendered vs client-rendered pages, APIs, and positions Django as a backend framework for building APIs.
...
> Covers setting up environment: upgrade to latest Python, use `pip`/`pip3` with Pipenv for dependency management, and install VS Code with Python extension for coding.
...
> Shows creating the first Django project with Pipenv (virtual environment, Pipfile), using `django-admin startproject`, understanding core files, switching to `manage.py`, and running the dev server.
...
> Shows how to configure VS Code to use the virtual environment’s Python interpreter, and set up the integrated terminal.
...
3. Now, whenever you'd open terminal in this VS code project, venv would automatically be activated by VS.
   ...
   (Again, remember project dir name would be added as prefix.)
...
> Explains Django apps, reviews default installed apps, shows how to create a new app (`playground`) with its standard structure, and register it in `INSTALLED_APPS`.
...
> Introduces Django views as request handlers, and shows creating a simple view function (`say_hello`) returning an `HttpResponse`.
...
> Explains mapping URLs to view functions by creating an app-level `urls.py`, using `path()` and `include()` to connect it with the project’s main `urls.py`, and routing `/playground/hello/` to return "Hello World".

Read docs.

## Using Templates

> Demonstrates using Django templates: creating `hello.html`, rendering it via `render(request, template, context)` to return dynamic HTML with variables and simple logic (`if`), while noting templates are rarely used in modern API-focused Django projects.

- Always namespace template files. Read TLDR in: [Gemini](https://g.co/gemini/share/73e20a46eaea).

- We can replace Django's default template engine with a preferred template engine if we want. (like Jinja)
...
> Shows how to debug Django apps in VS Code by creating a `launch.json` profile, using breakpoints, step over/into/out (F10/F11/Shift+F11), inspecting variables, and running apps with or without debugging.

- Setup: `Run and Debug` > `create a launch.json file` > `Python Debugger` > `Django` > `manage.py`.

  `launch.json` created in `.vscode`.

- Then, similar to normal Python debugger.

## Using Django Debug Toolbar

> Explains installing and configuring **Django Debug Toolbar** (install, add to `INSTALLED_APPS`, middleware, URLs, internal IPs) to analyze requests, settings, headers, and especially SQL queries during development.

Docs: [Installation](https://django-debug-toolbar.readthedocs.io/en/latest/installation.html)
...
- While adding to installed apps and middleware, use `if DEBUG` / `settings/dev.py`.

  See: [Order of Middlewares](/Notes/Part%203/7.%20Preparing%20for%20Production.md#order-of-middlewares)

### Not showing on some pages?

> "Note that the toolbar only shows up for pages with an HTML body tag"
> \- [Docs](https://django-debug-toolbar.readthedocs.io/en/latest/installation.html#troubleshooting)

**Solution:**

1. [Not working] Add following in settings:

    ```py
    DEBUG_TOOLBAR_CONFIG = {"SHOW_TOOLBAR_CALLBACK": lambda _: True}
    ```

2. [Working] Use [django-debug-toolbar-force](https://github.com/barseghyanartur/django-debug-toolbar-force).

   Installation: `pipenv install`; Add to `MIDDLEWARE`

   Usage: Append `/?debug-toolbar` to the url.

   (Make sure `/` is there. Without that, actual page content was not showing for me.)

Src: [AI Mode](https://share.google/aimode/FRdfuo0VEQWms3GUW)
