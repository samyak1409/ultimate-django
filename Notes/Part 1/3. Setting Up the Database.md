...
> Introduces Django-supported databases (SQLite, Postgres, MySQL, MariaDB, Oracle), recommends Postgres/MySQL for serious projects, and sets up switch from SQLite to MySQL for the course.
...
> SQLite is a serverless db. (file-based)

For production, most popular (relational): PostgreSQL, MySQL

> Mosh is using MySQL, but I'd use **Postgres** since it's **more popular** in Django.

## Creating Migrations

> Explains Django migrations—how `makemigrations` generates migration files for model changes (create, rename, add fields), naming/editing them, handling defaults/nullables, and importance of installed apps.
...
- Option 3: supply a one-off default value in the terminal (this is not saved in the model but only used for previous records, and stored in the current migration file)

Choice depends on our requirements.

<br>

What's a `models.SlugField()` and how exactly does it work? Read: [`Gemini`](https://g.co/gemini/share/faf64a82a49d)

## Running Migrations

> Covers running migrations with `migrate`, exploring the SQLite database/tables, tracking applied migrations, using `sqlmigrate` to view generated SQL, and naming/usage conventions for SQL/MySQL.
...
> This depends on the currently set DB in settings. (SQLite / PostgreSQL / etc.)

## Customizing Database Schema

> Shows customizing models with `Meta` options (table name, ordering, indexes), creating migrations for these changes, and why sticking to Django’s defaults is usually better.
...
<details>
<summary>Read again</summary>

- Migration files are like Git commits; reverting migrations follows a similar concept.

- To undo migrations up to a point:

  ```sh
  python manage.py migrate app_name XXXX
  ```

  This unapplies all migrations after `XXXX`.

- However, the code and migration files remain — so Django will reapply those unapplied migrations on the next `migrate`.

- Fix: either delete the migration files + undo model changes manually, or (better) use Git:

  ```sh
  git reset --hard HEAD~1
  ```

- To revert a migration in the middle of the chain, you must manually undo the model change and create a new migration that removes it.

</details>

## Installing ~~MySQL~~ PostgreSQL + Connecting to ~~MySQL~~ PostgreSQL

> Installing ~~MySQL~~ PostgreSQL, setting up root password, and connecting via GUI tool (DataGrip) to create the `storefront` database.
...
For just reading the data, use VS Code extension [SQL Tools](https://marketplace.visualstudio.com/items?itemName=mtxr.sqltools), which works well! (Doesn't support modifying.)  
...
> Install ~~`mysqlclient`~~ `psycopg`, update `settings.py` to use Postgres (DB name, host, user, password), and run migrations to create Django tables in the Postgres database.
...
> We can create an empty migration and use `migrations.RunSQL` to run custom queries or schema changes directly.
...
2. There, in `operations` list, use:
   ...
   If we don't provide `reverse_sql`, then we won't be able to undo the migration if we needed to using `migrate` cmd. Best practice: Always provide.

## Generating Dummy Data

> Used Mockaroo to generate and import dummy data (CSV/SQL) into MySQL tables for testing.
