...
> Introduction to Django managers and query sets — managers act as a DB interface, query sets are lazy objects for building complex queries, and they’re only evaluated when iterated, converted to a list, printed, etc.
...
`QuerySet` is not actually a `set` as the name might suggest. It actually works just like an `Iterator` object.  
Meaning, it evaluates the result (executes DB query) on the go when we actually access the value.

> PS: We can also say that expression is not evaluated until the resulting value is still a queryset.  
> e.g. `x = ModelName.objects.all()` - QS, so not evaluated yet.  
> `x[1:3]` - (Adds LIMIT clause) Still a QS, so not evaluated yet.  
> Now, if iteration (data access) / conversion to list / print etc. is done, then only the expression gets evaluated.
...
> Use `all()` (returns queryset), `get()` (single object, raises exception if not found or multiple found), or `filter().first()` / `filter().exists()` (safer alternatives to avoid exceptions).
...
**Imp:** Pay attention to `<object>.<attr>` usage (e.g. using `Product.description` when only `title` is loaded). Check the code in `playground.views.test_queries`.
...
> Some insights from my revision session:
>
> - `order_set` usage: Only in python code, `select_related()`, `prefetch_related()`.
> - `order` usage: Everywhere else, e.g., `F()`, `Count()`, etc.
...
**IMP: Use Strings to Initialize**

To truly avoid all floating-point issues, you must initialize Decimal objects using strings (e.g., Decimal('0.1')), not Python floats (e.g., Decimal(0.1)). If you pass a float to the Decimal constructor, the small initial binary approximation error is captured exactly, preserving the inaccuracy you were trying to avoid.
...
Mosh didn't explain anything, just read the section: [Caching and QuerySets](https://docs.djangoproject.com/en/5.2/topics/db/queries/#caching-and-querysets)

It's very important, must read.

> "Understanding how it works will allow you to write the most efficient code."

Here're the crisp notes from that section:

- A new QuerySet starts with an empty cache.
- First evaluation → DB query happens → results stored in the QuerySet’s cache.
- Further evaluations reuse the cached results (no new DB query).

Why it matters:

- Recreating QuerySets causes repeated queries.

  - Example: calling `Entry.objects.all()` twice = 2 DB hits.

- Use a saved QuerySet to avoid duplicate queries.

```py
qs = Entry.objects.all()
[p.headline for p in qs]  # Hits DB once
[p.pub_date for p in qs]  # Uses cache
```

When caching doesn't happen:

- Partial evaluations (slicing or indexing) do NOT populate cache.
  ```py
  qs[5]  # Always hits DB unless queryset already fully evaluated
  ```

Cache gets populated only when the _entire_ QuerySet is evaluated:
...
- Printing a QuerySet does NOT evaluate it fully (so no cache). Since `__repr__()` only fetches a slice.
